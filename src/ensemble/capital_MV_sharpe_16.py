...
# 1. Le os pesos diarios gerados pela otimizacao MV Sharpe da etapa 14.
...
# 5. Usa os pesos MV Sharpe apenas entre os ativos livres e renormaliza a soma.
...
def identificar_ativos_pesos(df):
    # Procura colunas como peso_PETR4_mv_sharpe e devolve PETR4.
    ativos = []

    for col in df.columns:
        match = re.match(r"peso_(.+)_mv_sharpe$", col)
...
def preparar_dados(caminho_pesos, caminho_targets):
    # Le os pesos do MV Sharpe e os retornos/targets da tecnica correspondente.
...
def processar_arquivo(
    ...
    print(f"\nDistribuindo capital MV Sharpe: {nome_arquivo}")
    ...
    # Ele comeca com data e metadados dos pesos calculados na etapa 14.
    saida = df[["data"]].copy()

    if "status_mv_sharpe" in df.columns:
        saida["status_mv_sharpe"] = df["status_mv_sharpe"]

    if "ativos_mv_sharpe" in df.columns:
        saida["ativos_mv_sharpe"] = df["ativos_mv_sharpe"]
    ...
    saida["capital_disponivel_inicio_mv_sharpe"] = 0.0
    saida["capital_alocado_novo_mv_sharpe"] = 0.0
    saida["capital_disponivel_fim_mv_sharpe"] = 0.0
    saida["capital_retido_mv_sharpe"] = 0.0
    saida["capital_total_mv_sharpe"] = 0.0
    saida["retorno_total_mv_sharpe"] = 0.0
    saida["soma_pesos_original_mv_sharpe"] = 0.0
    saida["soma_pesos_alocacao_mv_sharpe"] = 0.0
    ...
    for ativo in ativos_pesos:
        ...
        saida[f"peso_alocacao_{ativo}_mv_sharpe"] = 0.0
        ...
        saida[f"inicio_{ativo}_mv_sharpe"] = 0.0
        saida[f"fim_{ativo}_mv_sharpe"] = 0.0
        saida[f"retido_{ativo}_mv_sharpe"] = 0.0
        saida[f"retorno_{ativo}_mv_sharpe"] = 0.0
        saida[f"dias_{ativo}_mv_sharpe"] = 0

        # Ativos sem coluna de retorno ficam no output, mas nao entram na conta.
        if ativo in ativos_com_retorno:
            saida[f"status_{ativo}_mv_sharpe"] = "sem_posicao"
        else:
            saida[f"status_{ativo}_mv_sharpe"] = "sem_retorno"
    ...
    for i, row in df.iterrows():
        ...
        # Le os pesos originais da etapa de otimizacao MV Sharpe.
        pesos_originais = {
            ativo: valor_float(row.get(f"peso_{ativo}_mv_sharpe", 0.0))
            for ativo in ativos_pesos
        }
        ...
        for ativo, peso_alocacao in pesos_para_alocar.items():
            ...
            saida.loc[i, f"peso_alocacao_{ativo}_mv_sharpe"] = peso_alocacao
        ...
        for ativo in ativos_pesos:
            ...
            saida.loc[i, f"peso_{ativo}_mv_sharpe"] = peso_original
            ...
            saida.loc[i, f"inicio_{ativo}_mv_sharpe"] = capital_posicao
            saida.loc[i, f"fim_{ativo}_mv_sharpe"] = capital_fim
            saida.loc[i, f"retorno_{ativo}_mv_sharpe"] = retorno_ativo
            saida.loc[i, f"dias_{ativo}_mv_sharpe"] = dias_posicao
            saida.loc[i, f"status_{ativo}_mv_sharpe"] = status_posicao

            if manter_posicao:
                # Posicao segue aberta: o capital fica retido para o proximo dia.
                saida.loc[i, f"retido_{ativo}_mv_sharpe"] = capital_fim
        ...
        saida.loc[i, "capital_disponivel_inicio_mv_sharpe"] = capital_inicio
        saida.loc[i, "capital_alocado_novo_mv_sharpe"] = capital_alocado_novo
        saida.loc[i, "capital_disponivel_fim_mv_sharpe"] = capital_disponivel
        saida.loc[i, "capital_retido_mv_sharpe"] = capital_retido
        saida.loc[i, "capital_total_mv_sharpe"] = capital_total
        saida.loc[i, "retorno_total_mv_sharpe"] = retorno_total
        saida.loc[i, "soma_pesos_original_mv_sharpe"] = soma_pesos_original
        saida.loc[i, "soma_pesos_alocacao_mv_sharpe"] = sum(
...
def run_capital_mv_sharpe(
    weights_folder="./data/ensemble/14_mv_sharpe/",
    targets_folder="./data/ensemble/11_targets_por_tecnica/",
    output_folder="./data/ensemble/16_capital_mv_sharpe/",
    capital_inicial=100.0,
    dias_max_posicao=4
):
    # Executa a etapa 16 para todos os arquivos de pesos MV Sharpe.
    # Pasta de saida fica separada para nao mexer na metodologia 1/n.
    os.makedirs(output_folder, exist_ok=True)

    # Processa todos os CSVs de pesos gerados pelo MV Sharpe.
    arquivos = sorted([
        arquivo
    ...
    for arquivo in arquivos:
        # Cada arquivo da pasta 14 precisa de um arquivo equivalente na pasta 11.
        processar_arquivo(
    ...
    print("\nDistribuicao de capital MV Sharpe concluida.")
