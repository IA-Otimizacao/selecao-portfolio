...
def processar_arquivo(caminho, output_folder, janela=60):
    ...
    print(f"\nCalculando pesos minima variancia tecnicas: {nome_arquivo}")
    ...
    for i in range(1, len(df)):
        pesos, status_i = calcular_pesos_min_variancia(
            df,
            i,
            ativos,
            janela=janela,
    ...
    dados_saida = {
        "data": df["data"],
        "status_min_variancia": status,
        "ativos_min_variancia": ativos_usados,
    }

    for ativo in ativos:
        dados_saida[f"peso_{ativo}_min_variancia"] = pesos_por_ativo[ativo]
...
def run_otimizacao_mv_min_variancia_tecnicas(
    input_folder="./data/tecnicas/base_mv_sharpe_tecnicas_7/",
    output_folder="./data/tecnicas/mv_min_variancia_tecnicas_9/",
    janela=60,
):
    ...
    print("\nPesos MV minima variancia por tecnicas concluidos.")
